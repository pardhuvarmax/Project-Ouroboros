...
extern "C" int my_ieee_check(int32_t arg, int32_t arg2, int32_t arg3){
    if (arg == 31337)
      return 1;
    else
      return 0;
...
int WiFiScan::clearStations() {
  ...
  // Old versions had stations per AP; now global, so nothing else to clear.
  return num_cleared;
}
...
void WiFiScan::main(uint32_t currentTime)
{
  ...
  for (int i = 0; i < access_points->size(); i++) {
    if (access_points->get(i).selected) {
      // Fix: cast random() return (long) to uint8_t to prevent narrowing conversion
      this->broadcastCustomBeacon(
        currentTime,
        ssid{
          access_points->get(i).essid,
          static_cast<uint8_t>(random(1, 12)),
          {
