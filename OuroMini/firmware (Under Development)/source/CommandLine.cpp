...
void CommandLine::RunSetup() {
  ...
  Serial.println(F("\n\n--------------------------------\n"));
  Serial.println(F("         ESP32 OuroMini      \n"));
  Serial.println(String("            ") + cmd_version_number + "\n");
  Serial.println(F("       By: Team-Ouroboros\n"));
  Serial.println(F("--------------------------------\n\n"));
...
// List Stations
    ...
    char sta_mac_str[18];      // "AA:BB:CC:DD:EE:FF" + null terminator
    uint8_t sta_mac_bytes[6];  // raw 6-byte MAC
    int count_selected = 0;
    ...
    for (int x = 0; x < access_points->size(); x++) {
        ...
        for (int i = 0; i < access_points->get(x).stations->size(); i++) {
            String mac_str = webStations.get(access_points->get(x).stations->get(i)).mac;

            // Convert String MAC "AA:BB:CC:DD:EE:FF" -> uint8_t array
            int values[6];
            if (sscanf(mac_str.c_str(), "%x:%x:%x:%x:%x:%x", 
                       &values[0], &values[1], &values[2],
                       &values[3], &values[4], &values[5]) == 6) {
                for (int j = 0; j < 6; j++) sta_mac_bytes[j] = (uint8_t) values[j];
            } else {
                memset(sta_mac_bytes, 0, 6); // fallback to 00:00:00:00:00:00
            }

            // Convert back to string for printing
            snprintf(sta_mac_str, sizeof(sta_mac_str), "%02X:%02X:%02X:%02X:%02X:%02X",
                     sta_mac_bytes[0], sta_mac_bytes[1], sta_mac_bytes[2],
                     sta_mac_bytes[3], sta_mac_bytes[4], sta_mac_bytes[5]);

            // Print station info
            if (webStations.get(access_points->get(x).stations->get(i)).selected) {
                Serial.print("  [" + String(access_points->get(x).stations->get(i)) + "] ");
                Serial.print(sta_mac_str);
                Serial.println(" (selected)");
                count_selected++;
            } else {
                Serial.print("  [" + String(access_points->get(x).stations->get(i)) + "] ");
                Serial.println(sta_mac_str);
            }
        }
    }
    ...
    if (ap_sw != -1) {
      ...
      if (cmd_args.get(cl_sw + 1) == "all") {
        for (int i = 0; i < webStations.size(); i++) {
          if (webStations.get(i).selected) {
            // Unselect "selected" ap
            WebStation new_sta = webStations.get(i);
            new_sta.selected = false;
            webStations.set(i, new_sta);
            count_unselected += 1;
          }
          else {
            // Select "unselected" ap
      ...
      // Select specific Stations
        ...
        for (int i = 0; i < sta_index.size(); i++) {
          ...
          if (!this->inRange(webStations.size(), index)) {
            Serial.println("Index not in range: " + (String)index);
            continue;
          }
          if (webStations.get(index).selected) {
            // Unselect "selected" ap
            WebStation new_sta = webStations.get(index);
            new_sta.selected = false;
            webStations.set(index, new_sta);
            count_unselected += 1;
          }
          else {
            // Select "unselected" ap
